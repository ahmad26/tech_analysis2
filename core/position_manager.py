...
from core.position_tracker import PositionTracker, TrackedPosition
...
class PositionManager:
    def __init__(self, exchange, adapter: ExchangeAdapter, position_tracker: PositionTracker, alerter=None):
    ...
    def _fetch_candles(self, symbol: str, timeframe: str) -> pd.DataFrame | None:
        ...
        try:
            return fetch_ohlcv(self.exchange, self.adapter.market_symbol(symbol), timeframe, limit=50)
        except Exception:
    ...
    def _close(self, pos: TrackedPosition, price: float | None, reason: str) -> bool:
        ...
        try:
            order = self.adapter.market_order(self.exchange, pos.symbol, close_side, pos.contracts, reduce_only=True)
            ...
            if self.alerter:
                coins = self.adapter.amount_to_coins(self.exchange, pos.symbol, pos.contracts)
                pnl = (fill - pos.entry_price) * coins * (1 if pos.side == "long" else -1)
    ...
    def _is_flat(self, symbol: str) -> bool:
        ...
        try:
            positions = self.exchange.fetch_positions([self.adapter.market_symbol(symbol)])
            return all(
                abs(float(p.get("contracts") or 0)) <= 0
                for p in positions
                if self.adapter.base_symbol(p["symbol"]) == symbol
            )
        except Exception:
    ...
    def _replace_orders(self, pos: TrackedPosition, new_sl: float, new_tp: float, df) -> str:
        ...
        Placing BEFORE cancelling means a rejected new stop leaves the existing
        protection resting — the position is never left naked by a failed replace.
        Returns the SL-leg status so the caller can react:
        ...
        if new_sl > 0:
            try:
                sl_price = float(self.exchange.price_to_precision(self.adapter.market_symbol(pos.symbol), new_sl))
        ...
        if new_tp > 0:
            msym = self.adapter.market_symbol(pos.symbol)
            tp_price = float(self.exchange.price_to_precision(msym, new_tp))
            resting_tp = self._resting_tp_ids(pos.symbol)
            cur_tp = (float(self.exchange.price_to_precision(msym, pos.tp))
                      if pos.tp > 0 else 0.0)
            if resting_tp and tp_price == cur_tp:
                # Target unchanged and already resting — leave it. The maker TP is a
                # reduceOnly LIMIT, whose quantity is counted against the position's
                # reducible size the moment it rests; placing a SECOND full-size
                # reduceOnly LIMIT at the same level is rejected (-2022 on Binance).
                ...
                try:
                    tp_order = self.adapter.place_take_profit(self.exchange, pos.symbol, exit_side, pos.contracts, tp_price)
                    new_ids |= self._ids_of(tp_order)
                except Exception:
                    logger.exception(
    ...
    @staticmethod
    def _ids_of(order) -> set[str]:
        """All identifiers a freshly-placed order may be referenced by, so it can be
        excluded from the cancel sweep that follows a replace."""
    ...
    def _resting_tp_ids(self, symbol: str) -> set[str]:
        """Ids of take-profit orders currently resting for `symbol` (protective and
        non-stop). Used to keep the existing TP when a re-placement fails so the
        cancel sweep does not orphan it."""
        ids: set[str] = set()
        for order in self.adapter.fetch_protective_orders(self.exchange, symbol):
            if not order.is_stop:
                ids |= order.ids
        return ids

    def _cancel_resting_tp(self, symbol: str) -> None:
        """Cancel only the resting take-profit order(s) for `symbol`, leaving the
        stop-loss untouched. Required before re-placing the maker LIMIT TP at a NEW
        level: a resting reduceOnly LIMIT counts against the position's reducible
        size, so a second full-size reduceOnly LIMIT is rejected (-2022). The SL keeps
        protecting the position throughout."""
        for order in self.adapter.fetch_protective_orders(self.exchange, symbol):
            if order.is_stop:
                continue
            try:
                ...
                logger.warning("Could not cancel TP order %s for %s", order.ids, symbol)

    def _cancel_conditional(self, symbol: str, exclude_ids: set[str] | None = None) -> None:
        exclude_ids = exclude_ids or set()
        for order in self.adapter.fetch_protective_orders(self.exchange, symbol):
            if order.ids & exclude_ids:
                continue
            try:
                self.adapter.cancel_protective_order(self.exchange, symbol, order)
            except Exception:
                logger.warning("Could not cancel order %s for %s", order.ids, symbol)
