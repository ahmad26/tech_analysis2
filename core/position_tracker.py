...
class PositionTracker:
    def __init__(self, state_file: str = "position_state.json", symbol_normalizer=None):
        self.path = Path(state_file)
        # Maps an exchange-returned symbol to the canonical 'COIN/USDT' state key. Binance
        # default strips the ':USDT' settle suffix; OKX passes adapter.base_symbol so its
        # 'ETH/USD:USD-310404' positions key as 'ETH/USDT', matching scanned signals.
        self._normalize = symbol_normalizer or (lambda s: s.split(":")[0])
        ...
        with open(lock_path, "w") as lf:
            ...
            try:
                ...
                for p in current_positions:
                    ...
                    if contracts <= 0:
                        continue
                    symbol = self._normalize(p["symbol"])
